        ...
        <div className="section-container" style={{ position: 'relative', zIndex: 1 }}>
          <motion.div {...fadeUp(0.1)}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
              ...
              <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.68rem', fontWeight: 800, letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.55)' }}>{t("Uluslararası Programlar")}</span>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
              <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', fontWeight: 900, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--red)', background: 'rgba(230,25,54,0.15)', border: '1px solid rgba(230,25,54,0.3)', padding: '0.3rem 0.8rem', borderRadius: '4px' }}>
                {t("Yetkili Sınav Merkezi")}
              </span>
            </div>
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '1rem', color: 'rgba(255,255,255,0.65)', maxWidth: '600px', lineHeight: 1.85 }}>
              {t("Kolej Sancaktepe, Advanced Placement® sınavları için yetkili bir sınav merkezidir. Geniş bir yelpazede AP sınavı sunmaktan gurur duyuyoruz; hem kendi öğrencilerimiz hem de dışarıdan başvuran adaylar sınavlarımıza girebilir.")}
            </p>
          </motion.div>
        </div>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }} className="ap-intro">
            <motion.div {...fadeUp(0)}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.75rem' }}>
                ...
                <span className="section-tag">{t("Advanced Placement®")}</span>
              </div>
              <h2 className="section-title" style={{ marginBottom: '1.5rem' }}>{t("AP Nedir?")}</h2>
              <P>{t("Advanced Placement® (AP) Programı, öğrencilere henüz lisedeyken üniversite düzeyindeki çalışmalarla kendilerini sınama fırsatı verir. AP öğrencilerinin üniversiteye kaydolma, derslerinde daha iyi performans gösterme ve daha erken mezun olma olasılıkları daha yüksektir.")}</P>
              <P>{t("Bir öğrencinin transkriptindeki AP dersleri, öğrencinin üniversite düzeyindeki çalışmalara istekli ve akademik olarak hazır olduğunu gösterir.")}</P>
              <P>{t("AP kursları AP Sınavı ile tamamlanır. Sınavlar 5 puanlık ölçekte puanlanır; 3 ve üzeri puanlar, birçok üniversitede kredi kazanmaya veya giriş derslerini atlamaya imkân tanır.")}</P>
              ...
              >
                {t("AP Hakkında Daha Fazla Bilgi (College Board) →")}
              </a>
            </motion.div>
            <motion.div
        ...
        <div className="section-container">
          <motion.div {...fadeUp(0)} style={{ marginBottom: '2.5rem', textAlign: 'center' }}>
            <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(1.6rem,3vw,2.2rem)', fontWeight: 700, color: '#fff', marginBottom: '0.5rem' }}>
              {t("2025–2026 Önemli Tarihler")}
            </h2>
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.9rem', color: 'rgba(255,255,255,0.55)' }}>
              {t("Erken başvuru tavsiyesi: Kontenjan ve ücret avantajı için en geç Kasım başında başvurun.")}
            </p>
          </motion.div>
            ...
            {DATES.map((d, i) => (
              <motion.div key={i} {...fadeUp(i * 0.06)}>
                <div style={{
                  ...
                  <div style={{ fontFamily: 'var(--font-sans)', fontSize: '0.9rem', color: d.highlight ? '#fff' : 'rgba(255,255,255,0.7)', fontWeight: d.highlight ? 600 : 400 }}>{t(d.label)}</div>
                </div>
              </motion.div>
            ))}
          </div>
          <motion.div {...fadeUp(0.3)} style={{ marginTop: '2rem', textAlign: 'center' }}>
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.85rem', color: 'rgba(255,255,255,0.45)', maxWidth: '540px', margin: '0 auto', lineHeight: 1.75 }}>
              {t("Her sınav için sınav ücretinin son başvuru tarihinden sonra artacağını unutmayın. Kontenjanlar dolmadan başvurunuzu tamamlamanızı tavsiye ederiz.")}
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section style={{ padding: '6rem 0', background: 'var(--off-white)' }}>
        <div className="section-container">
          <motion.div {...fadeUp(0)} style={{ marginBottom: '3rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.75rem' }}>
              <div className="red-line" />
              <span className="section-tag">{t("Kayıt Süreci")}</span>
            </div>
            <h2 className="section-title">{t("AP Sınavlarına Nasıl")} <span>{t("Kayıt Yapılır?")}</span></h2>
          </motion.div>
            ...
            {STEPS.map((s, i) => (
              <motion.div key={i} {...fadeUp(i * 0.07)}>
                <div style={{
                  ...
                  <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.05rem', fontWeight: 700, color: 'var(--primary)', marginBottom: '0.75rem' }}>{t(s.title)}</h3>
                  <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.88rem', color: 'var(--text-mid)', lineHeight: 1.8, flex: 1 }}>{t(s.desc)}</p>
                  {s.action && (
          ...
          <motion.div {...fadeUp(0.2)} style={{ marginTop: '2rem' }}>
            <div style={{
              ...
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--primary)', marginBottom: '0.5rem' }}>{t("Dışarıdan Başvuran Öğrenciler")}</p>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.9rem', color: 'var(--text-mid)', lineHeight: 1.8 }}>
                {t("Dışarıdan gelen öğrencilerin kayıtlarını College Board sayfasından \"Exam Only\" seçeneğiyle tamamlamaları gerekmektedir. AP Classroom yalnızca onaylı AP dersleri veren okullara tam zamanlı kayıtlı öğrenciler için geçerlidir.")}
              </p>
            </div>
          </motion.div>
          ...
          <motion.div {...fadeUp(0.25)} style={{ marginTop: '2rem' }}>
            <div style={{
              ...
              <div>
                <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--grey)', marginBottom: '0.3rem' }}>{t("AP Sorularınız İçin")}</p>
              ...
              <div>
                <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--grey)', marginBottom: '0.3rem' }}>{t("Telefon")}</p>
