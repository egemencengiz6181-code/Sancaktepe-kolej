...
const GroupTitle = ({ icon, children, delay = 0 }) => (
  <motion.h3 {...fadeUp(delay)} style={{ fontFamily: 'var(--font-serif)', fontSize: '1.25rem', fontWeight: 700, color: 'var(--primary)', marginBottom: '1.25rem', marginTop: '2.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
    <span style={{ width: '30px', height: '30px', background: '#232e5c', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '0.85rem' }}>{icon}</span>
    {children}
  </motion.h3>
);

export default function IlkokulPDRPage() {
  return (
    <>
      <section style={{ background: 'linear-gradient(135deg, #0d1530 0%, #232e5c 60%, #2e3d7a 100%)', padding: '7rem 0 4.5rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="section-container" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.65 }} style={{ fontFamily: 'var(--font-sans)', fontSize: '1rem', color: 'rgba(255,255,255,0.6)', maxWidth: '580px', lineHeight: 1.85 }}>
            İlköğretim düzeyinde öğrenci, veli ve öğretmenlere yönelik psikolojik danışmanlık ve rehberlik çalışmaları.
          </motion.p>
        </div>
      </section>
      ...
      <section style={{ padding: '5.5rem 0', background: 'var(--off-white)' }}>
        <div className="section-container">
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            ...
            <motion.div {...fadeUp(0)} style={{ background: 'var(--white)', borderRadius: '14px', padding: '2rem 2.5rem', border: '1px solid var(--grey-light)', borderLeft: '5px solid #232e5c', marginBottom: '2.5rem', boxShadow: '0 4px 20px rgba(35,46,92,0.06)' }}>
            ...
            <InfoCard title="Oryantasyon Çalışmaları" delay={0.13} body='Anaokulundan ilkokula, ilkokuldan ortaokula geçen öğrencilerin ve ara sınıflarda kayıt yaptırmış yeni öğrencilerin hazırlığını güçlendirmek için "Okul Oryantasyon Programı" çerçevesinde çeşitli sınıf çalışmaları ve grup dinamiği etkinlikleri düzenlenir.' />
            <InfoCard title="Öğrenme Stilleri Çalışması" delay={0.15} body="Öğrencilerimizin bireysel öğrenme stilleri 5. ve 7. sınıf düzeylerinde uygulanarak öğrenci ve öğretmenlerle paylaşılır ve akademik süreçlere destek verilir. 2. sınıflarımızda da öğrenme stillerini belirleyecek çalışmalar yapılır." />
            ...
            <motion.div {...fadeUp(0.52)} style={{ background: 'var(--primary)', borderRadius: '14px', padding: '2rem 2.5rem', display: 'flex', gap: '2rem', alignItems: 'center', flexWrap: 'wrap', marginTop: '2rem' }}>
              <div style={{ flex: 1 }}>
