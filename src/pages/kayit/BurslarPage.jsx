...
const Section = ({ tag, title, children, delay = 0 }) => (
  <motion.div {...fadeUp(delay)} style={{ marginBottom: '3rem' }}>
    ...
    {title && <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.5rem', fontWeight: 700, color: 'var(--primary)', marginBottom: '1.5rem' }}>{t(title)}</h2>}
    <div style={{
      ...
      <section style={{ padding: '7rem 0', background: 'var(--off-white)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="section-container">
          ...
          <motion.div {...fadeUp(0)} style={{ marginBottom: '4rem' }}>
            <div style={{
              ...
              <div style={{ position: 'relative', zIndex: 1 }}>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: '0.68rem', fontWeight: 800, letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: '0.75rem' }}>{t("Bursluluk Programı")}</div>
                ...
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '1rem', color: 'rgba(255,255,255,0.75)', lineHeight: 1.85, maxWidth: '680px' }}>
                  {t("Kolej Sancaktepe, sosyal sorumluluk anlayışı çerçevesinde her yıl sınırlı sayıda öğrenciye Tam Eğitim Bursu sağlar. Bu burs ile öğrenci tüm eğitim dönemince okul ücretinden muaf tutulur.")}
                </p>
              </div>
            </div>
          </motion.div>
          ...
          <Section tag="Duyurular" delay={0.05}>
            <P>{t("Bursluluk sınavı tarihleri, başvuru koşulları ve ayrıntılı bilgiler her yıl Kolej Sancaktepe'nin resmi web sitesinde, sosyal medya kanallarında ve okul panosunda duyurulur.")}</P>
            <P>{t("Bursluluk sınavına başvuru koşulları akademik yıla göre güncellenebilmektedir; güncel bilgi için lütfen kayıt ofisiyle iletişime geçiniz.")}</P>
          </Section>
          ...
          <motion.div {...fadeUp(0.08)} style={{ marginBottom: '3rem' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.5rem', fontWeight: 700, color: 'var(--primary)', marginBottom: '2rem' }}>{t("Bursluluk Süreci")}</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }} className="burs-steps-grid">
              {PROCESS_STEPS.map((s, i) => (
                <div key={i} style={{
                  ...
                  <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '0.95rem', fontWeight: 700, color: 'var(--primary)', marginBottom: '0.5rem' }}>{t(s.title)}</h3>
                  <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.83rem', color: 'var(--text-mid)', lineHeight: 1.75 }}>{t(s.desc)}</p>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <Section tag="İzleme" title={t("Burslu Öğrencilerin İzlenmesi")} delay={0.1}>
            <P>{t("Burslu öğrenciler her dönem okul yönetimi, sınıf öğretmeni ve PDR uzmanı tarafından akademik, sosyal ve psikolojik gelişim açısından takip edilir.")}</P>
            <P>{t("İzleme görüşmeleri öğrencinin güçlü yönlerini ve geliştirmesi gereken alanlarını belirleyerek kişiye özel destek planları oluşturmayı amaçlar.")}</P>
          </Section>

          {/* Bursun Devamlılığı */}
          <Section tag="Devamlılık" title={t("Bursun Devamlılığı")} delay={0.12}>
            <P>{t("Tam Eğitim Bursu yalnızca akademik başarıyı değil, öğrencinin genel okul yaşamına katkısını ve değerlerimizle örtüşen tutum ve davranışlarını da kapsar.")}</P>
            <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.7rem', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--primary)', marginBottom: '0.5rem', marginTop: '1rem' }}>{t("Bursun Sürdürülmesi İçin Koşullar")}</p>
            <BulletList items={[
            ...
            <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.7rem', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--red)', marginBottom: '0.5rem', marginTop: '1.5rem' }}>{t("Bursun Sona Erebileceği Durumlar")}</p>
            <BulletList items={[
