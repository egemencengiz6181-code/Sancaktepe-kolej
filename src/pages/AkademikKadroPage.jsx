        ...
        <div className="section-container" style={{ position: 'relative', zIndex: 1 }}>
          <motion.div {...fadeUp(0.1)}>
            ...
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '1rem', color: 'rgba(255,255,255,0.65)', maxWidth: '560px', lineHeight: 1.85 }}>
              {t("Alanında uzman, öğrenciye adanmış ve sürekli gelişen bir kadroyla kaliteli eğitimin güvencesi.")}
            </p>
          </motion.div>
        </div>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.8fr', gap: '5rem', alignItems: 'center' }} className="kadro-intro-grid">
            <motion.div {...fadeUp(0)}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
                ...
                <span className="section-tag">{t("Yaklaşımımız")}</span>
              </div>
              <div style={{
                ...
                <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.3rem', fontWeight: 700, marginBottom: '0.75rem', lineHeight: 1.3 }}>
                  {t("Öğretmen,")}<br />{t("En Önemli Paydaş")}
                </h3>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.85rem', color: 'rgba(255,255,255,0.7)', lineHeight: 1.8 }}>
                  {t("TED, öğretmenleri eğitim kurumlarının hedeflerini gerçekleştirmede en temel paydaşlar olarak görür.")}
                </p>
              </div>
            </motion.div>
            ...
            <motion.div {...fadeUp(0.15)}>
              <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(1.6rem, 3vw, 2.4rem)', fontWeight: 700, color: 'var(--primary)', lineHeight: 1.25, marginBottom: '1.5rem' }}>
                {t("Nitelikli Öğretmen,")}<br /><span style={{ color: 'var(--red)' }}>{t("Kaliteli Eğitim")}</span>
              </h2>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: '1rem', color: 'var(--text-mid)', lineHeight: 1.95, marginBottom: '1.25rem' }}>
                {t("Türk Eğitim Derneği, öğretmenleri eğitim kurumlarının hedeflerini gerçekleştirmede en önemli paydaşlar olarak görmektedir. Bu nedenle de okullarında görev yapacak, alanında ve öğretmenlik mesleğinde yetkin öğretmenlerin seçiminde profesyonel bir yaklaşım sergilemektedir.")}
              </p>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: '1rem', color: 'var(--text-mid)', lineHeight: 1.95 }}>
                {t("TED Sancaktepe Kolejinde öğretmen ve yönetici istihdamı, Türk Eğitim Derneği Genel Merkezi'nin Ankara'da yürüttüğü yazılı ve mülakat süreçleriyle sağlanmaktadır. Düzenli aralıklarla tekrarlanan hizmet içi eğitimlerle öğretmenlerimizin mesleki gelişimleri en üst seviyede tutulmaktadır.")}
              </p>
            </motion.div>
          </div>
        </div>
        ...
        <div className="section-container">
          <motion.div {...fadeUp(0)} style={{ textAlign: 'center', marginBottom: '4rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', justifyContent: 'center', marginBottom: '0.75rem' }}>
              <div className="red-line" />
              <span className="section-tag">{t("Kadro Standartları")}</span>
              <div className="red-line" />
            </div>
            <h2 className="section-title" style={{ textAlign: 'center' }}>{t("Kadromuzu")} <span>{t("Tanımlayan Unsurlar")}</span></h2>
          </motion.div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.5rem' }} className="kadro-pillars-grid">
            {pillars.map((item, i) => (
              <motion.div key={t(item.title)} {...fadeUp(i * 0.08)}>
                <div style={{
                ...
                >
                  ...
                  <div>
                    <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '1rem', fontWeight: 700, color: 'var(--primary)', marginBottom: '0.5rem' }}>{t(item.title)}</h3>
                    <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.88rem', color: 'var(--text-mid)', lineHeight: 1.8 }}>{t(item.desc)}</p>
                  </div>
                </div>
              </motion.div>
            ))}
